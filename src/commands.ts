...
export function registerVeniceStatsCommands(
  ...
  pi.registerCommand("venice-wallet", {
    description: "Show or set your wallet address: /venice-wallet [0x...] or /venice-wallet clear",
    handler: async (args, ctx) => {
  ...
  pi.registerCommand("venice-budget", {
    description: "Show or set the stats polling budget (1–59 req/min, default 30): /venice-budget [1-59|reset]",
    handler: async (args, ctx) => {
      const raw = (args ?? "").trim();
  ...
  pi.registerCommand("venice-tz", {
    description: "Show or set the widget timezone (auto-detected by default): /venice-tz [timezone|reset]",
    handler: async (args, ctx) => {
      const raw      = (args ?? "").trim();
  ...
  pi.registerCommand("venice-billing-interval", {
    description: `Show or set the billing poll interval in seconds (${BILLING_INTERVAL_MIN}–${BILLING_INTERVAL_MAX}, default ${BILLING_INTERVAL_DEFAULT}): /venice-billing-interval [N|reset]`,
    handler: async (args, ctx) => {
      const raw     = (args ?? "").trim();
      ...
      if (!raw) {
        ...
        notify(ctx, `Billing poll interval: ${current}s ${source}\nUsage: /venice-billing-interval <${BILLING_INTERVAL_MIN}-${BILLING_INTERVAL_MAX}> or /venice-billing-interval reset`, "info");
        return;
      }
      if (raw === "reset") {
  ...
  pi.registerCommand("venice-widget", {
    description: "Manage the stats widget lock: /venice-widget claim — take over when the previous session is gone",
    handler: async (args, ctx) => {
      const sub = (args ?? "").trim();
      if (sub === "claim") {
